...
struct config_tA                         // Temperatura deposito
{
  int tempAset;
  byte tempAoff;
  byte tempAalarm; //
...
void SaveTempAToEEPROM()
{
 byte k = 66;
 EEPROM.write(705, k);  
  tempASettings.tempAset = int(setTempA * 10);
  tempASettings.tempAoff = int(offTempA * 10);
  tempASettings.tempAalarm = int(alarmTempA*10);
  EEPROM_writeAnything(514, tempASettings);
}
/*
...
void salvar_comederos_EEPROM()
{
  ...
  EEPROM_writeAnything(825, comederos);
}
/*
...
void ReadFromEEPROM()
{
  ...
  {
    EEPROM_writeAnything(514, tempASettings);
    setTempA = tempASettings.tempAset;
    setTempA /=10;
    offTempA = tempASettings.tempAoff;
    offTempA /=10;
    alarmTempA = tempASettings.tempAalarm;
    alarmTempA /= 10;
  }
 }
/*
...
{
  EEPROM_readAnything(825,comederos);
...
{
  byte k = EEPROM.read(849);

  if (k == 66)
  {
    for (byte i = 0; i < 9; i++)
    {
      outlets[i] = EEPROM.read(850 + i);
    }
  }
}
