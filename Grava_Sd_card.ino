...
void logtempgraf() //Grava dados no SD CARD para gerar  grafico de temperatura filtro.
{
...
void logtempdgraf() //Grava dados no SD CARD para gerar  grafico de temperatura acuario.
{
  if(NumMins(t.hour,t.min) == 1)
  {
  ...
  if(file.open("LOGTDDIA.TXT", O_CREAT | O_APPEND | O_WRITE))
  {
    if ((tempC<=1.0) || (tempC>99.9))
    {
      file.print("00.00");
  ...
  if ((t.hour == 23) && (t.min > 58))
  {
    if(file.open("LOGTDDIA.TXT", O_WRITE))
    {
      file.remove();
    }
  }
...
void logdengraf() //Grava dados no SD CARD para gerar  grafico de densidad do aquario.
{
  if(NumMins(t.hour,t.min) == 1)
  {
...
void check_arquivo_tempd_agua()
{
  int16_t n;
  char buf[8];
  int contador = 0;

  byte k = EEPROM.read(700);
  ...
  if(file.open("LOGTDDIA.TXT", O_CREAT | O_READ | O_APPEND | O_WRITE))
  {
    while ((n = file.read(buf, sizeof(buf))) > 0)
    {
      if(strlen(buf) == 5)
