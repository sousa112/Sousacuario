  ...
  else
  {
    switch (dispScreen)
    {
      ...
      case 1:     //------------------------------------Tela de menu-------------------------------------------------
        ...
        else if (checkButtons(tpaA[0], tpaA[1], tpaA[2], tpaA[3]) == true)          // Seleciona o menu sondas"
        {
          dispScreen = 41;
          clearScreen();
          procurar_sensores(true);
        }
        else if (checkButtons(dosA[0], dosA[1], dosA[2], dosA[3]) == true)          // Seleciona o menu "alimentador"
        {
          dispScreen = 45;
          clearScreen();
          comederoScreen();
       ...
       else if ((y>=iniC[1]) && (y<=iniC[3]) && (x>=iniC[0]) && (x<=iniC[2]))
      {
        ...
        else if (checkButtons(menU[0], menU[1], menU[2], menU[3]) == false)           // Seleciona o "Menu 2"
        {
          dispScreen = 1;
          clearScreen();
          menuScreen();
          ...
          if ((x >= 4) && (x <= 316))
          {
            ...
            if (oldLCT != LedChangTime)                        // Realçar período tocado
            {
              ...
              for (int i = 0; i < 8; i++)                          // Imprime valores do período.
              {
                ...
                myGLCD.printNumI( cor[cor_selecionada][k], (i * 38) + 12, 180);
                y_tocado = map(cor[cor_selecionada][k], 255, 0, 80, 180);
                setor_selecionado = i;
        ...
        else if ((y >= 80) && (y <= 180))
        {
          for (int i = 0; i < 8; i++) {
            if ((x >= (i * 38) + 5) && (x <= (i * 38) + 39))
            {
              int k = (LedChangTime * 8) + i;
              y_tocado = y;
              cor[cor_selecionada][k] = map(y_tocado, 180, 80, 0, 255);

              drawFillRect((i * 38) + 12, 200, (i * 38) + 36, 210, 0, 0, 0);
              setFont(SMALL, 255, 255, 255, 0, 0, 0);
              myGLCD.printNumI( cor[cor_selecionada][k], (i * 38) + 12, 195);
              setor_selecionado = i;
              desenhar_barras_periodo ();
            }
          }
        ...
        else if ((y >= 50) && (y <= 75) && (periodo_selecionado == true))       // Botão +
        {
          for (int i = 0; i < 8; i++)
          {
            if ((x >= (i * 38) + 5) && (x <= (i * 38) + 39))
            {
              ...
              setFont(SMALL, 255, 255, 255, 0, 0, 0);
              myGLCD.printNumI( cor[cor_selecionada][k], (i * 38) + 12, 195);
              y_tocado = map(cor[cor_selecionada][k], 255, 0, 80, 185);
              setor_selecionado = i;
              desenhar_barras_periodo ();
            }
          }
        }
        else if ((y >= 194) && (y <= 219)  && (periodo_selecionado == true)) // Botão -
        {
          for (int i = 0; i < 8; i++)
          {
            if ((x >= (i * 38) + 5) && (x <= (i * 38) + 39))
            {
              waitForIt((i * 38) + 5, 194, (i * 38) + 39, 219);
              int k = (LedChangTime * 8) + i;
              cor[cor_selecionada][k] -= 1;
              delay(50);
      ...
      case 9: //  -----------------------------------------Tela escolher graficos-----------------------------------------
        /*if (checkButtons(orP[0], orP[1], orP[2], orP[3]) == true)
        {
          dispScreen = 13;
          clearScreen();          
          PHRScreen(); //GRAFICO PH DEPOSITO
        }*/
        if (checkButtons(phR[0], phR[1], phR[2], phR[3]) == true)
        {
          dispScreen = 13;
          clearScreen();
          PHAScreen(); //GRAFICO PH URNA
        }
        else if (checkButtons(tempG[0], tempG[1], tempG[2], tempG[3]) == true)
        {
          dispScreen = 11;
          clearScreen();
          tempgScreen(); //GRAFICO TEMPERATURA URNA
        }
        else if (checkButtons(phA[0], phA[1], phA[2], phA[3]) == true)
        {
          dispScreen=12;
          clearScreen();
          tempDgScreen();  //GRAFICO TEMPERATURA DEPOSITO
        }
      ...
      /* if (checkButtons(orP[0], orP[1], orP[2], orP[3]) == true)
        {
          dispScreen=15; // pantalla configuracion parametros ph deposito
          clearScreen();
          config_phR_Screen(true);
        }*/
        if (checkButtons(phR[0], phR[1], phR[2], phR[3]) == true)
        {
          dispScreen = 18; // pantalla configuracion parametros ph acuario
          clearScreen();
          config_phA_Screen(true);
        }
        else if (checkButtons(phA[0], phA[1], phA[2], phA[3]) == true)
        {
          dispScreen=55; //pantalla configuracion temperatura agua deposito
          clearScreen();
          tempAScreen(true);
        }
        else if (checkButtons(tempG[0], tempG[1], tempG[2], tempG[3]) == true)
        {
          dispScreen = 4; //pantalla configuracion temperatura agua urna
          clearScreen();
          tempScreen(true);
        }
...
*/
      case 30:  // ----------------------------------------- Digitar a senha --------------------------------------------
        ...
        {
          if ((stCurrent[0] == senha [0]) && (stCurrent[1] == senha [1]) && (stCurrent[2] == senha [2]) && (stCurrent[3] == senha [3])  && stCurrentLen <= 4) /* && (stCurrent[4] == senha [4]) && (stCurrent[5] == senha [5])*/
          {
            stCurrentLen = 0;
            ...
            myGLCD.print(buffer, 1, 192); // tabela_textos[173] = "SENHA CORRETA"
          ...
          else if (stCurrentLen <= 4)
          {
            ...
            for (byte i = 0; i < 3; i++)
            {
              ...
              myGLCD.print(buffer, 1, 192); // tabela_textos[174] = "SENHA INCORRETA!"

              delay(500);
              myGLCD.setColor(0, 0, 0);
              myGLCD.fillRect(1, 170, 320, 210);
              delay(500);
            }
          ...
          else if (stCurrentLen > 4)
          {
            ...
            for (byte i = 0; i < 3; i++)
            {
              ...
              myGLCD.print(buffer, 35, 192); // "MAXIMO 4 DIGITOS!"
      ...
      case 38: // -------------------------------- Timers --------------------------------------------------------------
        ...
        else if (checkButtons(7, 22, 56, 220) == true)           // Timer 1
        {
          temporizador = 0;
          dispScreen = 39;
          clearScreen();
        ...
        else if (checkButtons(71, 22, 120, 220) == true)           // Timer 2
        {
          temporizador = 1;
          dispScreen = 39;
          clearScreen();
        ...
        else if (checkButtons(135, 22, 184, 220) == true)           // Timer 3
        {
          temporizador = 2;
          dispScreen = 39;
          clearScreen();
        ...
        else if (checkButtons(199, 22, 248, 220) == true)           // Timer 4
        {
          temporizador = 3;
          dispScreen = 39;
          clearScreen();
        ...
        else if (checkButtons(263, 22, 312, 220) == true)           // Timer 5
        {
          temporizador = 4;
          dispScreen = 39;
          clearScreen();
      ...
      case 44://---------------------------------------- Potência pré-definida para os leds ----------------------------
        if (horario_alterado == false)
        {
          ...
          if (pre_definido_ativado_t == 1) // Permite alterar valores somente se a função estiver ativada.
          {
            ...
            {
              if (checkButtons(210, 190, 240, 220) == true) // PWM +
              {
                pwm_pre_definido_t += 1;
                pre_definido();
              }
              else if (checkButtons(280, 190, 310, 220) == true) // PWM -
              {
                pwm_pre_definido_t -= 1;
                pre_definido();
              }
      ...
      if ((x>=85) && (x<=235) && (y>=94) && (y<=114))      //Presiona comer ahora!!
        {
         ...
         myGLCD.print("Alimentando", 118, 100); 
         ...
         myGLCD.print("Alimentar ahora!", 103, 100);  
     ...
     /*   if (checkButtons(menU[0], menU[1], menU[2], menU[3]) == true)           // volta ao menu
      ...
      if (comedero ==4)      
        ...
        if (alimentacao_erro == false)
        {
          ...
          {
            desligar_wavemaker_temp += 5;
            if (desligar_wavemaker_temp > 480)
            {
              desligar_wavemaker_temp = 5;
            }
            alimentadorScreen_2();
          ...
          {
            desligar_wavemaker_temp -= 5;
            if (desligar_wavemaker_temp < 5)
            {
              desligar_wavemaker_temp = 480;
            }
            alimentadorScreen_2();
          }
