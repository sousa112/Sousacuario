...
void menuScreen()
{
  ...
  printButton(buffer, tpaA[0], tpaA[1], tpaA[2], tpaA[3]); // sondas de temperatura

  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[0])));
  printButton(buffer, dosA[0], dosA[1], dosA[2], dosA[3]); // alimentador
...
*/
  ...
  printButton(buffer, leST[0], leST[1], leST[2], leST[3]); // "ALTERAR"

  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[82])));
  printButton(buffer, savE[0], savE[1], savE[2], savE[3]); // "SALVAR EEPROM"
  ...
  printButton(buffer, leWB[0], leWB[1], leWB[2], leWB[3]); // tabela_textos[210] = "MAIS"
}
...
void graficoScreen()//------------------------------------------------------------tela =9
{
  ...
  printButton(buffer, tempG[0], tempG[1], tempG[2], tempG[3]); // // TEMPERATURA URNA

  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[97])));
  printButton(buffer, phA[0], phA[1], phA[2], phA[3]); // // TEMPERATURA DEPOSITO

  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[94])));
  printButton(buffer, phR[0], phR[1], phR[2], phR[3]); // // PH URNA
/*
  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[95])));
  printButton(buffer, orP[0], orP[1], orP[2], orP[3]); // // PH DEPOSITO
...
void parametroScreen()//-------------------------------------------------------------------tela =14
{
  ...
  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[94])));
  printButton(buffer, phR[0], phR[1], phR[2], phR[3]); // "PH AQUARIO"
/*
  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[95])));
  printButton(buffer, orP[0], orP[1], orP[2], orP[3]); // "PH DEPOSITO"
...
void TimerScreen()
{
  ...
  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[126])));
  printButton(buffer,7, 55, 56, 94);      // Botão ON 1 

  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[126])));
  printButton(buffer,71, 55, 120, 94);    // Botão ON 2

  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[126])));
  printButton(buffer,135, 55, 184, 94);   // Botão ON 3

  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[126])));
  printButton(buffer,199, 55, 248, 94);   // Botão ON 4

  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[126])));
  printButton(buffer,263, 55, 312, 94);   // Botão ON 5

  myGLCD.setColor(255, 255, 255);  // Retângulos ao redor dos horários
  myGLCD.drawRoundRect(7, 99, 56, 137);      // Botão ON 1 
  myGLCD.drawRoundRect(71, 99, 120, 137);    // Botão ON 2 
  myGLCD.drawRoundRect(135, 99, 184, 137);   // Botão ON 3 
  myGLCD.drawRoundRect(199, 99, 248, 137);   // Botão ON 4 
  myGLCD.drawRoundRect(263, 99, 312, 137);   // Botão ON 5  

  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[127])));
  printButton(buffer, 8, 142, 55, 185);      // Botão OFF 1 

    strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[127])));
  printButton(buffer, 72, 142, 119, 185);    // Botão OFF 2 

  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[127])));
  printButton(buffer, 136, 142, 183, 185);   // Botão OFF 3 

  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[127])));
  printButton(buffer, 200, 142, 247, 185);   // Botão OFF 4 

  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[127])));
  printButton(buffer, 264, 142, 311, 185);   // Botão OFF 5 

  myGLCD.setColor(255, 255, 255);  // Retângulos ao redor dos horários
  myGLCD.drawRoundRect(7, 190, 56, 219);     // Botão OFF 1 
  myGLCD.drawRoundRect(71, 190, 120, 219);   // Botão OFF 2 
  myGLCD.drawRoundRect(135, 190, 184, 219);  // Botão OFF 3 
  myGLCD.drawRoundRect(199, 190, 248, 219);  // Botão OFF 4 
  myGLCD.drawRoundRect(263, 190, 312, 219);  // Botão OFF 5

  myGLCD.setBackColor(0, 0, 0);

  for (byte i = 0; i < 5; i++)
  {
    if (on_hora_e[i] < 10) // Timer on
    {
      myGLCD.print("0", 12 + (i * 64), 111);
      myGLCD.printNumI(on_hora_e[i], 21 + (i * 64), 111);
    }
    else
    {
      myGLCD.printNumI(on_hora_e[i], 12 + (i * 64), 111);
    }
    myGLCD.print(":", 29 + (i * 64), 111);

    if (on_minuto_e[i] < 10)
    {
      myGLCD.print("0", 37 + (i * 64), 111);
      myGLCD.printNumI(on_minuto_e[i], 46 + (i * 64), 111);
    }
    else
    {
      myGLCD.printNumI(on_minuto_e[i], 37 + (i * 64), 111);
    }

    if (off_hora_e[i] < 10) // Timer off
    {
      myGLCD.print("0", 12 + (i * 64), 203);
      myGLCD.printNumI(off_hora_e[i], 21 + (i * 64), 203);
    }
    else
    {
      myGLCD.printNumI(off_hora_e[i], 12 + (i * 64), 203);
    }

    myGLCD.print(":", 29 + (i * 64), 203);

    if (off_minuto_e[i] < 10)
    {
      myGLCD.print("0", 37 + (i * 64), 203);
      myGLCD.printNumI(off_minuto_e[i], 46 + (i * 64), 203);
    }
    else
    {
      myGLCD.printNumI(off_minuto_e[i], 37 + (i * 64), 203);
    }
  }
  strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[1])));
...
void procurar_sensores(boolean refreshAll = false)
{
  ...
  if (refreshAll == true)
  {
    ...
    myGLCD.print(buffer, 5, 174); // "ASSOCIAR TEMP. DO DISSIPADOR A"

    strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[222])));
    myGLCD.print(buffer, 5, 214); // "ASSOCIAR TEMP. AMBIENTE A"
  }
...
#ifdef USE_FAHRENHEIT
    myGLCD.print("(68.0 - 95.0)", 11, 70);
#else
   myGLCD.print("(20 - 35)", 115, 70);
#endif
...
#else
    myGLCD.print("(20 - 50)", 115, 170);  // Intervalo
#endif
...
void reduzir_potencia(boolean refreshAll = false)
{
  ...
  myGLCD.printNumI(potR_t, 220, 136);
}
...
void pre_definido(boolean refreshAll = false)
{
  ...
  if (refreshAll == true)
  {
    ...
    printButton("+", 210, 190, 240, 220, true); // Todos pwm +
    printButton("-", 280, 190, 310, 220, true); // Todos pwm -
...
void comederoScreen()
{
  ...
  if (comedero1==0)                                 //Feeding Time 1 Button 
    ...
    { myGLCD.setColor(0, 255, 0);
      ...
      myGLCD.print("HORA DE COMER 1", 24, 25);
      setFont(LARGE, 255, 255, 255, 0, 0, 0);          
      myGLCD.printNumI(comedero1H,30,55,2);
      myGLCD.print(":",60,55);
      myGLCD.printNumI(comedero1M,70,55,2);
  ...
  if (comedero2==0)                                 //Feeding Time 2 Button
    { myGLCD.setColor(255, 0, 0);
      ...
      setFont(SMALL, 255, 255, 255, 255, 0, 0);
      myGLCD.print("HORA DE COMER 2", 184, 25); 
      setFont(SMALL, 255, 0, 0, 0, 0, 0);
      strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[28])));
    ...
    { myGLCD.setColor(0, 255, 0);
      ...
      myGLCD.print("HORA DE COMER 2", 184, 25);
      setFont(LARGE, 255, 255, 255, 0, 0, 0);          
      myGLCD.printNumI(comedero2H,200,55,2);
      myGLCD.print(":",230,55);
      myGLCD.printNumI(comedero2M,240,55,2);      
      }          
  if (comedero3==0)                                 //Feeding Time 3 Button
    { myGLCD.setColor(255, 0, 0);
      myGLCD.fillRoundRect(5, 168, 155, 188);
      setFont(SMALL, 255, 255, 255, 255, 0, 0);
      myGLCD.print("HORA DE COMER 3", 24, 173);
      setFont(SMALL, 255, 0, 0, 0, 0, 0);
      strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[28])));
    ...
    { myGLCD.setColor(0, 255, 0);
      ...
      myGLCD.print("HORA DE COMER 3", 24, 173);
      setFont(LARGE, 255, 255, 255, 0, 0, 0);          
      myGLCD.printNumI(comedero3H,30,130,2);
      myGLCD.print(":",60,130);
      myGLCD.printNumI(comedero3M,70,130,2);      
      }          
  if (comedero4==0)                                 //Feeding Time 4 Button
    { myGLCD.setColor(255, 0, 0);
      myGLCD.fillRoundRect(165, 168, 315, 188);
      setFont(SMALL, 255, 255, 255, 255, 0, 0);
      myGLCD.print("HORA DE COMER 4", 184, 173); 
      setFont(SMALL, 255, 0, 0, 0, 0, 0);
      strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[28])));
    ...
    { myGLCD.setColor(0, 255, 0);
      ...
      myGLCD.print("HORA DE COMER 4", 184, 173);
      setFont(LARGE, 255, 255, 255, 0, 0, 0);          
      myGLCD.printNumI(comedero4H,200,130,2);
      myGLCD.print(":",230,130);
      myGLCD.printNumI(comedero4M,240,130,2);
      }      
   ...
   if (comedero == 1)//******************************* Pantalla comedero  1 *******************************
{
    ...
    {
      strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[147])));
      printButton_green (buffer, domI[0]-20, domI[1], domI[2]+20, domI[3]); // Verde > ON
    } 
    else if (comedero1_Temp == 0) 
    {
      strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[148])));
      printButton_red(buffer, domI[0]-20, domI[1], domI[2]+20, domI[3]); // Rojo > OFF 
    }
  }
...
*/
//--------------------------------------------- menu control temperatura deposito -------------------------------- tela = 55
void tempAScreen(boolean refreshAll=false)
{
  ...
  if (refreshAll)
  {
    ...
    tempA2beS = setTempA;
    tempA2beO = offTempA;
    tempA2beA = alarmTempA;

    strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[24])));
    printHeader(buffer); //tabela_textos[24] "CONTROLE DE TEMPERATURA DA AGUA DEPOSITO"
  ...
  setFont(LARGE, 255, 255, 255, 0, 0, 0);
  myGLCD.printNumF(tempA2beS, 1, 130, 40);
  myGLCD.printNumF(tempA2beO, 1, 140, 90);
  myGLCD.printNumF(tempA2beA, 1, 140, 140);
}
