...
/*
   if (outlets_changed[2] == true)
    ...
    SetRele(desativarFonte,LOW);
    }

if (outlets[2] == 0) //MODO AUTOMATICO SELECCIONADO
   {
    ...
    SetRele(desativarFonte,HIGH);
    }
   }
 else if (outlets[2] == 1) //MODO ON SELECCIONADO 
   {
   bitWrite(status_parametros,7,1); 
   SetRele(desativarFonte,HIGH);
   }
  else if (outlets[2] == 2) //MODO OFF SELECCIONADO
   {
   bitWrite(status_parametros,7,0);  
   SetRele(desativarFonte,LOW);
   }
